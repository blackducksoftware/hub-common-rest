...
public class NTLMEngineImpl {
    // code taken from https://github.com/square/okhttp/issues/206
    public static class NTLMEngineException extends Exception {
        private static final long serialVersionUID = -3377147784647159384L;

        public NTLMEngineException(final String message) {
            super(message);
        }

        public NTLMEngineException(final String message, final Throwable e) {
            super(message, e);
        }
    }
    ...
    private Charset credentialCharset = DEFAULT_CHARSET;
    ...
    /**
     * Creates the type 3 message using the given server nonce. The type 3 message includes all the information for authentication, host, domain, username and the result of encrypting the nonce sent by the server using the user's password
     * as the key.
     *
     * @param user
     ...
     */
    void setCredentialCharset(final Charset credentialCharset) {
     ...
     */
    static byte[] getLMResponse(final String password, final byte[] challenge)
            throws NTLMEngineException {
     ...
     */
    static byte[] getNTLMResponse(final String password, final byte[] challenge)
            throws NTLMEngineException {
     ...
     */
    static byte[] getNTLMv2Response(final String target, final String user, final String password,
            final byte[] challenge, final byte[] clientChallenge, final byte[] targetInformation)
            throws NTLMEngineException {
        final byte[] ntlmv2Hash = ntlmv2Hash(target, user, password);
        final byte[] blob = createBlob(clientChallenge, targetInformation);
     ...
     */
    static byte[] getLMv2Response(final String target, final String user, final String password,
            final byte[] challenge, final byte[] clientChallenge) throws NTLMEngineException {
        final byte[] ntlmv2Hash = ntlmv2Hash(target, user, password);
     ...
     *            The random 8-byte client challenge.
     *
     * @return The NTLM2 Session Response. This is placed in the NTLM response field of the Type 3 message; the LM response field contains the client challenge, null-padded to 24 bytes.
     */
    static byte[] getNTLM2SessionResponse(final String password, final byte[] challenge,
    ...
    static class NTLMMessage {
         ...
         *
         * @param length
         *            the maximum length of the response to prepare, not including the type and the signature (which this method adds).
         */
    ...
    static class Type3Message extends NTLMMessage {
        ...
        Type3Message(String domain, String host, final String user, final String password, final byte[] nonce,
            ...
            try {
                if (targetInformation != null && target != null) {
                    final byte[] clientChallenge = makeRandomChallenge();
                    ...
                    if ((type2Flags & FLAG_NEGOTIATE_NTLM2) != 0) {
                        ...
                        final byte[] clientChallenge = makeNTLM2RandomChallenge();
        ...
        String getResponse() {
            ...
            final int finalLength = sessionKeyOffset + 0;
    ...
    /**
     * Cryptography support - MD4. The following class was based loosely on the RFC and on code found at http://www.cs.umd.edu/~harry/jotp/src/md.java. Code correctness was verified by looking at MD4.java from the jcifs library
     * (http://jcifs.samba.org). It was massaged extensively to the final form found here by Karl Wright (kwright@metacarta.com).
     */
    static class MD4 {
